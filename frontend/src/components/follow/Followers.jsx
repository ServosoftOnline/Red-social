...
export const Followers = () => {
  ...
  // Importo auth desde el contexto
  const {auth} = useAuth();
  const idUsuarioLogado = auth._id;
  ...
  const getInitialUsers = async () => {
    ...
    try {
      ...
      // Realizo la petición
      const request = await fetch(GLOBAL.url + "follow/followers/" + idUser , {
        method: "GET",
        headers: {
      ...
      if (data.status === "success" && data.docs) { 
        ...
        // 1. Mapeamos para obtener el objeto 'user' 'followed'.
        const usersFollowers = data.docs.map(followRelation => followRelation.user);

        // 2. FILTRAMOS CUALQUIER ELEMENTO NULL/UNDEFINED (Esto arregla el error)
        const validUsers = usersFollowers.filter(Boolean);
  ...
  const getMoreUsers = async (pageToLoad) => {
    ...
    try {
       ...
       // followRoutes.get('/followers/', auth, followers); 

      // Hago la peticion
      const request = await fetch(GLOBAL.url + "follow/followers/" + idUser + '/' + pageToLoad, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          "Authorization": token
  ...
  return (
    <>
      <header className="content__header">
        <h1 className="content__title content__title__followers">Gente que sigue a {profile.name} {profile.surname}</h1>
      </header>
